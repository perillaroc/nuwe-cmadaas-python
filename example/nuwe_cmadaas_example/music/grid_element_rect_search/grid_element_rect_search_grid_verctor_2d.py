...
def query(username: str, password: str, server_id: str, client_config: str):
    interface_id = "getNafpVectorEleGridByTimeAndLevelAndValidtime"
    ...
    result = client.callAPI_to_gridVector2D(interface_id, params, server_id)

    print("return code:", result.request.error_code)
    print("return message:", result.request.error_message)

    print(np.array(result.u_datas))
    print(np.array(result.v_datas))
